...
def _preview_dataframe(df: pd.DataFrame, currency_cols: Optional[list[str]] = None) -> pd.DataFrame:
    """Return a display-only copy with null-like values shown as empty cells.

    Currency columns (if provided) are formatted with a leading '$' for display
    only; the underlying df remains numeric.
    """
    if df.empty:
        return df
...
def _run_pipeline(
    ...
    return {
        ...
        "currency_columns": meta.get("currency_columns", []),
        "file_name": file_name,
    }
...
def main() -> None:
    ...
    ui.badges(
        badge_list=[("CSV", "secondary"), ("AI", "secondary"), ("Clean", "secondary")],
    ...
    if df_raw is not None and file_name is not None:
        ...
        with tab_report1:
            ...
            score_raw = quality_raw.get("score", 0.0)
            ui.metric_card(
                title="Overall score",
                content=str(score_raw),
                description="Raw data quality",
                key="metric_raw_score",
            )
            comps = quality_raw.get("components", {}) or {}
            st.caption("Components: completeness, consistency, validity, uniqueness")
            st.json(
                {
        ...
        # Use the native Streamlit primary button here so it respects the
        # active theme (including dark mode) more reliably than the shadcn
        # button variant.
        if st.button("Run Preprocessing and Analyze", type="primary", key="btn_run_preprocessing"):
        ...
        if res is None:
            ...
            with tab_clean:
                st.dataframe(
                    _preview_dataframe(df_after.head(50), res.get("currency_columns")),
                    width="stretch",
                )
                st.caption(
            ...
            with tab_report3:
                ...
                if not q:
                    ...
                    score_clean = q.get("score", 0.0)
                    ui.metric_card(
                        title="Overall score",
                        content=str(score_clean),
                        description="After preprocessing",
                        key="metric_clean_score",
                    )
            ...
            if not issues:
                ...
                if next_batch:
                    with st.expander(
                        ...
                        for iss in next_batch:
                            ui.badges(
                                badge_list=[(iss.category.name, "secondary")],
                                class_name="flex gap-1",
                                key=f"issue_badge_{iss.id}",
                            )
                            st.caption(f"`{iss.column}` · {iss.message}")
                else:
                    st.caption(
                ...
                if st.button("Generate Recommendations", type="primary", key="btn_generate_recs"):
                    if next_batch:
                    ...
                    if not matched:
                        ...
                        if not actionable:
                            ...
                            for issue, rec in actionable:
                                ...
                                # Keep this visually lightweight in dark mode by
                                # using a bordered container instead of a white card.
            ...
            st.caption("Export your cleaned dataset as CSV.")
            st.download_button(
