...
def build_recommendation_prompt(issues: List[Dict[str, Any]]) -> str:
    ...
    return (
        ...
        "- For dates: convert clearly parseable date text to a canonical ISO date (YYYY-MM-DD), including\n"
        "  straightforward language/format normalization (e.g. month names in other languages such as\n"
        "  'The 6th of marzo, year 1951' -> '1951-03-06').\n"
        "- For dates: NEVER guess when the value is ambiguous or invalid. If a date string does not represent a real\n"
        "  calendar date in a clear format (e.g. '1984-02-34', '1976-13-24', '01/16-03'), map it to null instead of\n"
        "  inferring an intended date.\n"
        "- For clearly corrupted text (encoding glitches, stray bytes, obvious typos), prefer mapping to a canonical\n"
        "  value or to null instead of inventing new values.\n"
        "- It IS appropriate to consolidate synonymous categorical labels into a single canonical label when the\n"
        "  meaning is clearly the same (for example, mapping 'Not Rated' and 'Approved' to 'Unrated').\n"
        "- Keep changes as minimal and local as possible; do not infer information that is not present in the\n"
        "  provided values.\n\n"
